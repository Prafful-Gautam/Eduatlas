...
message: string;
otp: string;
  constructor(private fb: FormBuilder, private otpService: OtpService, private toasterService: NbToastrService, 
  ...
  getOtp(){
    ...
    if(!this.display){
      ...
      if(this.displayPass)
      {
        ...
        this.otpService.varifyOtp(param, this.newPassword).subscribe(data => {
          console.log(data);
    ...
    if (this.display)
    {
      ...
      this.otpService.getOtp(this.phone, param).subscribe(data => 
      {
        console.log(data);
        this.display = false;
